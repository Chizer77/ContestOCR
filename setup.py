...
def quiz():
    ...
    while True:
        ...
        if exist(window_config.loc) and exist(window_config.size):
            beginClick(window_config)
            while True:
                idx = 0
                while idx < 5:
                    time.sleep(3 + random.uniform(-0.5, 1))
                    ques, idx = extract_question_structure(ocr, window_config, True)
                    ...
                    for qid, qdata in question_data.items():
                        if qdata["question"] == ques["question"]:
                            ...
                            isClick = False
                            while isClick == False:
                                isClick = clickAnswer(qdata["answer"], window_config)
                            break
                    if repeat_question:
                        continue
                    ...
                    isClick = False
                    while isClick == False:
                        isClick = clickAnswer(ans["answer"], window_config)

                time.sleep(2 + random.uniform(-0.5, 1))
                iscontinue = False
                while iscontinue == False:
                    iscontinue = continueClick(window_config)
